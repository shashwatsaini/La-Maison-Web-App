...
from application.models import ServiceProfessionals, CustomerRequests, ServiceRequests
...
def sendServiceProfessionalReports():
    ...
    with smtplib.SMTP_SSL(app.config['SMTP_SERVER'], 465) as smtp_server:
        ...
        for service_professional in service_professionals:
            ...
            service_requests = ServiceRequests.query.filter(ServiceRequests.serviceProfessional_id == service_professional.email)
            ...
            service_requests = CustomerRequests.query.filter(CustomerRequests.serviceProfessional_id == service_professional.email, CustomerRequests.status == 0)
